 ...
 */
export async function handleNonStreamingResponse({ providerResponse, provider, model, statisticsModel, sourceFormat, targetFormat, body, cacheKeyBody, cacheVariant, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, semanticCacheEnabled, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, customToolNames, trackDone, appendLog, pxpipe, reqTag, requestId, log }) {
  ...
  saveUsageStats({ provider, model: recordedModel, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, requestId, silent: true });
