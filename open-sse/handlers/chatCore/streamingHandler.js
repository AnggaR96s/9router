 ...
 */
export function buildOnStreamComplete({ provider, model, statisticsModel, connectionId, apiKey, requestStartTime, body, stream, finalBody, translatedBody, clientRawRequest, pxpipe, reqTag, requestId, log }) {
  const recordedModel = statisticsModel || model;
  ...
  const onStreamComplete = (contentObj, usage, ttftAt) => {
    ...
    saveUsageStats({ provider, model: recordedModel, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, requestId, label: "STREAM USAGE", silent: true });
