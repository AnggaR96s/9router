...
function getContentBlocksFromMessage(msg, toolNameMap = new Map()) {
  ...
  if (msg.role === ROLE.TOOL) {
    ...
    // OpenAI-style reasoning has no Claude equivalent other than a thinking block:
    // without this the assistant trace is dropped before the request leaves. It must
    // come first (Anthropic requires thinking blocks ahead of text) and it mirrors
    // openai-to-gemini.js, which pushes the same value as a thought part.
    // formats/claude.js attaches the provider signature where one is required.
