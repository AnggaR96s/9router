...
// The free tier answers 403 FreeTierError unless bash and read are on the wire, with or
// without caller tools — verified against /zen/v1/responses: caller tools alone 403,
// caller tools + decoys 200, decoys alone 200, no tools at all 403.
const DECOY_NAMES = ["bash", "read"];
const expectDecoysAppended = (tools) => {
  expect(tools.slice(0, TOOLS.length)).toEqual(TOOLS);
  expect(tools.slice(TOOLS.length).map((t) => t.name)).toEqual(DECOY_NAMES);
};
...
describe("opencode Free 1.3 tool_choice auto-only", () => {
  ...
  it.each([
    ...
    for (const model of [FREE_13, `${FREE_13}(max)`]) {
      ...
      expectDecoysAppended(out.tools);
  ...
  it("giữ auto và absent; tools/input nguyên vẹn", () => {
    ...
    expectDecoysAppended(autoOut.tools);
    ...
    expectDecoysAppended(absentOut.tools);
  ...
  it("wire: execute gửi choice auto tới /zen/v1/responses", async () => {
    ...
    expectDecoysAppended(sent.tools);
