// Route -> header metadata for the dashboard top bar.
// Kept free of JSX so the route map can be unit tested directly.
...
export const getPageInfo = (pathname) => {
  ...
  if (pathname.includes("/system-status"))
    return {
      title: "System Status",
      description: "Live host, storage and V8 runtime telemetry",
      icon: "monitoring",
      breadcrumbs: [],
    };
  if (pathname.includes("/pxpipe"))
    return {
      title: "PXPIPE",
      description: "PXPIPE bridge status, image pipeline activity and logs",
      icon: "image",
      breadcrumbs: [],
    };
