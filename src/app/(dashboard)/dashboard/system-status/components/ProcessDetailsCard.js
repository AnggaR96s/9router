...
export default function ProcessDetailsCard({ data, onTriggerGc }) {
  ...
  const gcAvailable = application.gcAvailable !== false;
      ...
      <Card className="p-5 border-black/5 dark:border-white/5 bg-surface space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-black/5 dark:border-white/5 pb-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h4 className="text-sm font-semibold text-text-main truncate">V8 Engine & Heap Memory</h4>
          </div>
          <Button
            size="sm"
            variant="outline"
            onClick={handleGc}
            disabled={gcLoading || !gcAvailable}
            title={
              gcAvailable
                ? "Request a V8 garbage collection"
                : "Unavailable: this Node runtime was not started with --expose-gc"
            }
        ...
        {!gcAvailable && (
          <p className="flex items-start gap-1.5 text-[11px] leading-relaxed text-text-muted">
            ...
            <span className="min-w-0">
              Garbage collection is not exposed in this Node runtime. Start the server with{" "}
              <code className="font-mono">--expose-gc</code> to enable this action.
            </span>
          </p>
        )}

        {gcResult && (
          <p className="flex items-start gap-1.5 text-[11px] leading-relaxed text-primary animate-fade-in">
            <span className="material-symbols-outlined text-[14px] shrink-0">check_circle</span>
            <span className="min-w-0 break-words">{gcResult}</span>
          </p>
        )}
