...
function getStorageAndDbStats(adapter) {
  let dbSizeBytes = 0;
  let walSizeBytes = 0;
  let shmSizeBytes = 0;
  let connected = false;
  ...
  try {
    ...
    if (adapter) {
      connected = true;
      // Nama tabel mengikuti skema 9router di fork ini, bukan skema dispatcher.
      const tables = {
      ...
      for (const [key, table] of Object.entries(tables)) {
        try {
          counts[key] = adapter.get(`SELECT COUNT(*) as c FROM ${table}`)?.c || 0;
        } catch {}
      }
    }
  } catch {
...
export async function getSystemTelemetry() {
  ...
  const adapter = await getAdapter().catch(() => null);
  const storage = getStorageAndDbStats(adapter);
  ...
  return {
    ...
    application: {
      ...
      // The dashboard disables the GC button when the runtime cannot collect.
